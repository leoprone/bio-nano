   ...
   /*    
      ...
      function mostrarTablaEditable(json) {
        /*   
          ...
          tablaTitulo.textContent =  fileNameElement.textContent;; // actualiza el título de la TABLA
         ...
         tablaTitulo.style.color="white";
         tablaTitulo.style.zIndex="20";
         tablaTitulo.style.fontSize="25px";
         tablaTitulo.style.marginBottom ="30px";
         tablaTitulo.style.backgroundColor ="gray";
         tablaTitulo.style.padding ="5px 0";
   ...
   /*  fastaInput.addEventListener("change", (event) => {
        ...
        reader.onload = function (event) {
            ...
            lines.forEach(line => {
                ...
                if (line.startsWith(">")) {
                    if (id) {
                        // Agregar el ID y la secuencia al CSV
                        csvContent += `${id},${sequence}\n`;
                    }
                    id = line.substring(1).trim(); // Extraer el ID
                    sequence = ""; // Reiniciar la secuencia
                } else {
                    sequence += line.trim(); // Construir la secuencia
                }
            });
    
            // Agregar la última secuencia al CSV
            if (id) {
                csvContent += `${id},${sequence}\n`;
            }
...
console.log("content", csvContent);
            inicioFasta(csvContent); // Aquí inicias tu flujo completo
            document.getElementById("fasta-btn").textContent="Cambiar Archivo FASTA";
        };
...
/*     fastaInput.addEventListener("change", (event) => {
        ...
        reader.onload = function (event) {
            const fastaText = event.target.result;
            const lines = fastaText.split(/\r?\n/); // Manejar saltos de línea Windows/Mac/Linux
            let csvContent = "ID,Secuencia\n"; // Encabezados del CSV
            let id = "";
            let sequence = "";
            ...
            lines.forEach(line => {
                ...
                if (line.startsWith(">")) {
                    ...
                    id = line.substring(1).trim(); // Extraer el ID sin ">"
                    sequence = ""; // Reiniciar la secuencia
                } else {
                    // Añadir la línea limpiando caracteres no válidos y en mayúsculas
                    sequence += line.toUpperCase().replace(/[^A-Z]/g, "");
                }
            });
...
/* 
    fastaInput.addEventListener("change", (event) => {
        const fastaInput = event.target;
        const files = fastaInput.files;
        if (files.length === 0) {
            alert("Por favor selecciona un archivo FASTA.");
            return;
        }
        const rows = [["ID", "Otro", "Secuencia"]]; // Encabezado del CSV
       ...
       Array.from(files).forEach((file) => {
        ...
        reader.onload = function (event) {
            ...
            let csvContent = "ID, otro,Secuencia\n";
            let id = null;
            let otro= "";
            let sequence = "";
            ...
            lines.forEach(line => {
                ...
                if (line === "") return; // Saltar líneas vacías
                   ...
                   if (line.startsWith(">")) {
                        if (id !== null) {
                            csvContent += `${id},${otro},${sequence}\n`;
                        }

                        let headerParts = line.substring(1).trim().split(",");
                        id = headerParts[0].trim(); // Primer término como ID
                        otro = headerParts.length > 1 ? headerParts.slice(1).join(",").trim() : ""; // Los demás como "Otro"
                        
                        sequence = "";
                    }

                    else {
                    // Acumular secuencia (puede contener caracteres no estándar)
            ...
            rows.push([id, otro, sequence]); // Escapar el texto completo
            if (rows.length === files.length + 1) {
            convertirACsv(rows);
            }
    
            // Mostrar el resultado en consola para verificar
          
    
            // Si quieres descargar el archivo CSV:
            // descargarCSV(csvContent);
    
            // Si tu flujo espera un string CSV:
...
// Función para convertir el array en formato CSV y descargarlo
/* function convertirACsv(rows) {
    ...
    function convertirACsv(rows) {
               ...
               /*  const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
                ...
                link.href = URL.createObjectURL(blob);
                link.download = "cv_consolidado.csv";
                link.style.display = "block";
                link.innerText = "Descargar CSV"; */
                
               /*  fileNameElement.classList.remove("display-none"); */
                // Pasar el contenido al siguiente flujo en la app
...
function deleteRow(id) {
    ...
    if (rowElement) {
     ...
     /*  descargarTablaComoCsv.style.zIndex="50";
      descargarTablaComoCsv.style.fontSize="30px";
      descargarTablaComoCsv.style.color="black"; */
     ...
     /*  descargarTablaComoJson.style.zIndex="50";
      descargarTablaComoJson.style.fontSize="30px";
      descargarTablaComoJson.style.color="black";
      */
      ...
      function guardarComoCSV(datos) {
          ...
          link.href = URL.createObjectURL(blob);
          link.download = "editados.csv"; // Nombre del archivo
      ...
      saveAsCsv.addEventListener("click", ()=> {
         guardarComoCSV(resultadoFiltrado);
      });
 ...
 // 🔍 Extrae todas las claves únicas del JSON
    ...
    // 🖱️ Cuando se hace clic para descargar
    function descargarJSONFiltrado() {
     /*  const checkboxes = document.querySelectorAll('#claveOpciones input[type="checkbox"]');
      const clavesSeleccionadas = Array.from(checkboxes)
        .filter(checkbox => checkbox.checked)
        .map(checkbox => checkbox.value); */
       ...
       while (clavesSeleccionadas.length === 0) {
         
            descargarJson.textContent="Seleccionar Campos";
            /* alert("Seleccioná al menos una clave para continuar."); */
            switchBtn.classList.remove("display-none");
            claveOpciones.classList.remove("display-none");
            clavesSeleccionadas = clavesJsonElegidas();
      ...
      const datosFiltrados = filtrarClaves(resultadoFiltrado, clavesSeleccionadas);
    ...
    // 🚀 Inicialización
    /* const claves = obtenerClavesUnicas(datos);
    generarCheckboxes(claves);  */
 ...
 function cargaDirectamenteCSV() {
     ...
     function funcionesDelDescagadorSinonimos(){
      ...
      function mostrarEditorSinonimos(){
       ...
       /*  document.getElementById("tabla-sinonimos").style.height ="auto"; */
        crearListaSinonimos.addEventListener("click", ()=> {
            ...
            /* crearListaSinonimos.addEventListener("mouseover", ()=>{
                const p =document.createElement("p");
                p.textContent = "Guardó la lista actual?";
                p.style.color = "blue";
                ...
                crearListaSinonimos.appendChild(p);
            }); */
        /*     crearListaSinonimos.addEventListener("click", ()=>{
                const input = document.createElement("input");
                input.type = "text";
                input.placeholder="Si no se guardó, se borrará la lista anterior";
                input.style.color= "white";
                input.style.fontSize = "20px";
                crearListaSinonimos.appendChild(input);

                if(input.value.toLowerCase()=== "s"){
                    listaSinonimos.innerHTML=""; 
                    arregloTotal = [];
        
                    mostrarEditorSinonimos(arregloTotal);
                }else{
                   document.getElementById("nombreArchivo").style.borderColor="red";
                }
            }); */
        });
      ...
      /* funcionesDelEditorSinonimos(); */




      
      const mostrarBuscadorBtn = document.getElementById("mostrar-buscador-btn");
    
      function funcionesDelBuscador(){
        
        mostrarBuscadorBtn.addEventListener("click", ()=>{
            mapSection.classList.remove("display-none");
            
            mostrarBuscadorBtn.style.display = "none";
     ...
     /*  actualizarBuscadorBtn.style.fontSize="30px";
      actualizarBuscadorBtn.style.color="white";
      actualizarBuscadorBtn.style.zIndex="30";
      actualizarBuscadorBtn.style.backgroundColor="black"; */
...
/* 
    ...
    function resaltarTexto(texto, busqueda, contexto = 100) {
        ...
        palabras.forEach(palabra => {
            ...
            while ((match = regex.exec(texto)) !== null) {
                const inicio = Math.max(0, match.index - Math.min(contexto, texto.length));
                const fin = Math.min(texto.length, match.index + match[0].length + Math.min(contexto, texto.length));

               /*  const inicio = Math.max(0, match.index - contexto);
                const fin = Math.min(texto.length, match.index + match[0].length + contexto); */
    
                const fragmento = texto.slice(inicio, match.index) +
                                  `<mark>${match[0]}</mark>` +
                                  texto.slice(match.index + match[0].length, fin);
      ...
      function imprimirResultados(resultadoFiltrado, opciones) {
              ...
              resultadoFiltrado.forEach(item => {
                   ...
                   opciones.forEach(opcion =>{ 
                        // para cada CAMPO
                        fragmentos = resaltarTexto(item[opcion].Highlighted || item[opcion], inputBuscador.value, 50);
                       
                        let index = opciones[opcion];
                        ...
                        fragmentos.forEach(fragmento =>{
                            ...
                            if(index % 2 === 0){
                                contenedorResultados.style.backgroundColor="gray";
                            }
                        })
...
const noMostrarBtn = document.getElementById("noMostrar");
let mostrar= "";
 ...
 */
     ...
     /*    const nombreArchivoBuscador = document.getElementById("nombre-archivo-buscador");
           nombreArchivoBuscador.textContent = fileNameElement.textContent;
           nombreArchivoBuscador.style.fontSize ="25px";
           nombreArchivoBuscador.style.zIndex ="20";
           nombreArchivoBuscador.style.color ="white";
           nombreArchivoBuscador.style.textAlign ="center";
          
           nombreArchivoBuscador.style.display ="inline-block";
           nombreArchivoBuscador.style.width = "90%";
           nombreArchivoBuscador.style.padding =" 5px 0"
           nombreArchivoBuscador.style.margin ="40px auto";
           nombreArchivoBuscador.style.backgroundColor ="gray"; */
...
/* 
      function mostrarSinónimos(arregloTotal){
        listaSinonimos.innerHTML =""; // reseteamos la lista
        arregloTotal.forEach(grupo =>{
            const li = document.createElement("li");
            li.textContent = grupo.join(", ");
            listaSinonimos.appendChild(li);
        });
...
/* 
            if (termino && sinonimosInput.length > 0) {
                    diccionarioSinonimos[termino] = sinonimosInput.toLowerCase().split(",").map(s => s.trim());



                    arreglo = Object.entries(diccionarioSinonimos).map(([termino, lista]) => [termino, ...lista]);
                    ...
                    listaSinonimos.innerHTML = html;
                    /*  listaSinonimos.innerHTML += `<li>${termino}: ${sinonimos[termino].join(", ")}</li>`; 
                    listaSinonimos.style.color = "white";

            } else {
                alert("Por favor ingrese un término y al menos un sinónimo");
            } */

                        
            // Convertimos el diccionario termino: [sinónimo1, sinon2, ...] en 
            // arreglo de arreglos [[]] "gruposDePalabras" = [[termino1,sinon1.1, sinon1.2,...], [termino2, sinon2.1, sinon2.2, ...]]
            // es decir, un arreglo de arreglos que incluyen terminos y sus sinónimos
           /*  gruposDePalabras = transformarDiccionario(sinonimos); */
...
/* 
        function transformarDiccionario (diccionario) {
            return Object.entries(diccionario).map(([termino, sinonimos]) => [termino, ...sinonimos]);
        }
...
/*     
        function buscarConSinonimos(datos, termino, campos) {
            const sinonimosDelTermino = sinonimos[termino.toLowerCase()] || [termino];
            return datos.filter(row =>
                campos.some(campo =>
                    sinonimosDelTermino.some(sinonimo =>
                        (row[campo] || "").toLowerCase().includes(sinonimo.toLowerCase())
                    )
                )
            );
        } */